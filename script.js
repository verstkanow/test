...
class Api {
  ...
  get_api_path(object, template) {
    return template.replace(/%\w+%/g, (word) => {
      const elem = word.replace(/%/g, "");
      return typeof object[elem] === "string" ? object[elem].replace(/\s/g, "") : object[elem];
    });
  }
}
...
let api_path_templates = ["/api/items/%id%/%name%", "/api/items/%id%/%role%", "/api/items/%id%/%salary%"];
